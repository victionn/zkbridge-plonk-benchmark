...
import { UnprovenTransaction, SucceedEntirely, getImpureCircuitIds } from '@midnight-ntwrk/midnight-js-types';
import { assertDefined, assertIsContractAddress, parseCoinPublicKeyToHex, parseEncPublicKeyToHex, fromHex, assertUndefined, toHex } from '@midnight-ntwrk/midnight-js-utils';
import { getZswapNetworkId, getRuntimeNetworkId, getLedgerNetworkId } from '@midnight-ntwrk/midnight-js-network-id';
import { UnprovenOutput, UnprovenTransient, UnprovenInput, UnprovenOffer, ContractState, ContractCallsPrototype, ContractCallPrototype, communicationCommitmentRandomness, ContractDeploy, QueryContext, VerifierKeyRemove, MaintenanceUpdate, signData, ReplaceAuthority, VerifierKeyInsert, StateValue, ContractOperationVersion, ContractOperationVersionedVerifierKey, ContractMaintenanceAuthority, partitionTranscripts, PreTranscript, LedgerParameters } from '@midnight-ntwrk/ledger';
...
const zswapStateToOffer = (zswapLocalState, encryptionPublicKey, params) => {
    const unprovenOutputs = new Map(zswapLocalState.outputs.map((output) => [
        ...
        createUnprovenOutput(output, encryptionPublicKey)
    ]));
    ...
    zswapLocalState.inputs.forEach((qualifiedCoinInfo) => {
        ...
        if (unprovenOutput) {
            unprovenTransients.set(serializedCoinInfo, UnprovenTransient.newFromContractOwnedOutput(qualifiedCoinInfo, 1, unprovenOutput));
            ...
            unprovenInputs.set(serializedCoinInfo, UnprovenInput.newContractOwned(qualifiedCoinInfo, 1, params.contractAddress, params.zswapChainState));
        }
    });
...
const toLedgerQueryContext = (queryContext) => new QueryContext(StateValue.decode(queryContext.state.encode()), queryContext.address);
