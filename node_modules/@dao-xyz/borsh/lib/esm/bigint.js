...
export function writeBufferLEBigInt(num, width, buffer, offset) {
...
export function writeUInt32LE(value, buf, offset) {
...
export function writeUInt16LE(value, buf, offset) {
    checkInt(value, 0, 0xffff, 1);
    buf[offset] = value;
    buf[offset + 1] = value >>> 8;
}
export const writeBigUint64Le = (bigIntOrNumber, buf, offset) => {
    let lo, hi;
...
export const readBigUInt64LE = (buf, offset) => {
...
export const readUInt16LE = (buffer, offset) => {
    const first = buffer[offset];
...
export const checkInt = (value, min, max, byteLength) => {
